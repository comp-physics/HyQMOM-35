% Save ALL intermediate values after step 5
clear all;
close all;
...
params = struct();
params.Np = Np;
params.tmax = 0.074;  % Should give us ~5 steps (where Julia previously exploded)
params.Kn = Kn;
params.Ma = Ma;
...
fprintf('Running MATLAB for ~5 steps (t=0.074)...\n');
script_dir = pwd;
...
save('matlab_step5_all_values.mat', 'result', '-v7.3');

fprintf('\n=== MATLAB Step 5 Complete ===\n');
...
fprintf('\nSample values at cell (7,13):\n');
fprintf('  rho = %.15e\n', rho(7,13));
fprintf('  u   = %.15e\n', u(7,13));
fprintf('  v   = %.15e\n', v(7,13));
fprintf('  T   = %.15e\n', T_field(7,13));
fprintf('  M(1:5) = [%.6e, %.6e, %.6e, %.6e, %.6e]\n', ...
        M_final(7,13,1), M_final(7,13,2), M_final(7,13,3), ...
        M_final(7,13,4), M_final(7,13,5));

% Check for any NaN or Inf
if any(isnan(M_final(:))) || any(isinf(M_final(:)))
    fprintf('\n⚠️  WARNING: NaN or Inf detected in MATLAB results!\n');
else
    fprintf('\n✅ No NaN or Inf in MATLAB results\n');
end

fprintf('\nSaved to matlab_step5_all_values.mat\n');
